...
class CustomAccountAdapter(DefaultAccountAdapter):
    def get_email_confirmation_url(self, request, emailconfirmation):
        ...
        user_role = request.POST.get("roles", [])

        if "vendor" in user_role:
            url = f"{settings.VENDOR_URL}verify-email/{key}/"
        elif "rider" in user_role:
            url = f"{settings.RIDER_URL}verify-email/{key}/"
        else:
