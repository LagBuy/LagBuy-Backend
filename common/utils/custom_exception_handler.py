...
def custom_exception_handler(exc, context):
    ...
    if isinstance(exc, UserAlreadyExist) and response is not None:
        ...
        if user_email:
            try:
                ...
                logger.error(f"Failed to send user already exists email to {user_email}: {email_error}")
    ...
    error_message = (f"\n\t\t\tINTERNAL SERVER ERROR:\n"
                    ...
                    f"Logged in user\t: {request.user} [{request.user.email if request.user else 'Anonymous'}]\n"
